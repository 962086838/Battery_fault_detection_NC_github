...
def get_best_performance_data(total_err_scores, gt_labels, topk=1):
    ...
    total_topk_err_scores = np.sum(np.take_along_axis(total_err_scores, topk_indices, axis=0), axis=0)
    # print('in eval total_topk_err_scores shape', total_topk_err_scores.shape) # in eval total_topk_err_scores shape (2044,)
    np.save('total_topk_err_scores.npy', total_topk_err_scores)
